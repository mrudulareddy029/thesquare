...
export default function ContactProvider({ children }: { children: ReactNode }) {
  ...
  // ✅ Simple email check (enough for UI validation)
  ...
  const handleNameChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.trim().length > 0) setErrors((prev) => ({ ...prev, name: false }));
  };
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-6 md:px-10 pt-8 md:pt-10 pb-4">
              <h4 className="text-2xl md:text-[34px] font-bold text-[#7ca232]">
                Contact Us
              </h4>
              ...
              <form className="flex flex-col gap-4" onSubmit={handleSubmit} noValidate>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex flex-col">
                    <input
                      placeholder="Name"
                      className={`w-full px-5 py-3 rounded-xl bg-white border border-transparent outline-none shadow-sm text-black text-base placeholder:text-gray-400 ${
                        errors.name ? "border-red-500" : "focus:border-[#7ca232]"
                      }`}
                      value={formData.name}
                  ...
                  <div className="flex flex-col">
                    <input
                      ...
                      type="email"
                      className={`w-full px-5 py-3 rounded-xl bg-white border border-transparent outline-none shadow-sm text-black text-base placeholder:text-gray-400 ${
                        errors.email ? "border-red-500" : "focus:border-[#7ca232]"
                      }`}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex flex-col">
                    <input
                      ...
                      className={`w-full px-5 py-3 rounded-xl bg-white border border-transparent outline-none shadow-sm text-black text-base placeholder:text-gray-400 ${
                        errors.phone ? "border-red-500" : "focus:border-[#7ca232]"
                      }`}
                    />
                    {errors.phone && (
                      <p className="text-red-500 text-[10px] mt-1 pl-1">
                  ...
                  <div className="flex flex-col">
                    <div
                      ...
                      className={`relative flex items-center justify-between w-full px-5 py-3 rounded-xl bg-white shadow-sm cursor-pointer border border-transparent text-black text-base ${
                        errors.date ? "border-red-500" : "focus:border-[#7ca232]"
                      }`}
                    >
                ...
                <div className="relative pt-2">
                  <label className="flex items-start gap-3 cursor-pointer group">
                    <div className="relative mt-1 flex-shrink-0 w-4 h-4 flex items-center justify-center">
                      ...
                      <svg
                        className="w-3 h-3 text-black scale-0 peer-checked:scale-100 transition-transform pointer-events-none z-10 stroke-[4px]"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        strokeLinecap="round"
                    ...
                    <span className="text-[10px] leading-tight text-gray-600 select-none mt-1">
                      I authorize representatives of IRA Square to contact me. This consent overrides
                      DNC/NDNC.
                    </span>
                  </label>
                </div>
                ...
                <button
                  type="submit"
                  className="w-full bg-[#7ca232] text-white text-lg font-semibold py-4 rounded-[20px] shadow-lg transition-all active:scale-95"
                >
                  Book a Visit
                </button>
              </form>
